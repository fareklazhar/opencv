...
with open(os.path.join(ScriptHome, "camera_build.conf"), "rt") as ConfFile:
    HomeDir = os.getcwd()
    ...
    for s in ConfFile:
        ...
        print(f"Building {MakeTarget} for {Arch}")
        BuildDir = os.path.join(HomeDir, f"{MakeTarget}_{Arch}")
        ...
        os.chdir(HomeDir)
        CameraLib = os.path.join(BuildDir, "lib", Arch, f"lib{MakeTarget}.so")
        if (os.path.exists(CameraLib)):
            try:
                shutil.copyfile(
                    CameraLib,
                    os.path.join(
                        "..", "3rdparty", "lib", Arch, f"lib{MakeTarget}.so"
                    ),
                )
